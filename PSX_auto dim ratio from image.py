...
5. Auto Full-screen, Sửa lỗi UI trên màn DPI cao, Thuật toán tự né đè thẻ vàng.
6. Phóng to núm kéo gấp đôi & fix lẹm nút Popup Edit.
7. KHÓA HÌNH VẼ: Bảo vệ hạng mục không bị kéo lệch sau khi vẽ xong.
8. Tách Layer khi xuất ảnh, Tag chữ vàng luôn nằm TRÊN CÙNG, đè lên mọi khung đỏ.
9. FIX LỖI LẸM CHỮ: Nút bấm tự động co giãn (Auto-expand), chia đều không gian, không bị cắt chữ.
"""
...
class AppDoKichThuoc:
    ...
    def init_ui(self):
        ...
        lbl_hint = tk.Label(canvas_frame, text="💡 Kéo chuột vẽ ô | Nhấp chọn ô để hiện núm căn chỉnh | Hạng mục BỊ KHÓA sẽ hiển thị màu cam/vàng",
        ...
        # Thay width cứng bằng padx để nút tự do co giãn
        self.btn_calib_lock = tk.Button(calib_grid, text="🔒 Khóa", font=("Segoe UI", 8, "bold"),
                                        bg="#00b894", fg="white", cursor="hand2", relief=tk.FLAT, padx=8,
        ...
        frame_measure = tk.LabelFrame(right_panel, text=" 📏 Bước 2: Thông tin đối tượng đo (Khóa Tỉ Lệ 🔒) ",
        ...
        # Dòng 1: Tên Hạng mục
        tk.Label(meas_grid, text="Tên Hạng Mục:", font=("Segoe UI", 9, "bold"), bg="#f5f6fa").grid(row=0, column=0, sticky="w", pady=3)
        ...
        # Thay width cứng bằng padx để nút tự do co giãn
        self.btn_meas_lock = tk.Button(meas_grid, text="🔒 Tỉ lệ", font=("Segoe UI", 8, "bold"),
                                       bg="#00b894", fg="white", cursor="hand2", relief=tk.FLAT, padx=8,
        ...
        btn_add = tk.Button(frame_measure, text="✔ Cập Nhật Số Đo (Enter)", font=("Segoe UI", 10, "bold"),
                            bg="#0984e3", fg="white", cursor="hand2", relief=tk.FLAT, command=self.apply_manual_edit_and_update)
        btn_add.pack(fill=tk.X, pady=(4, 2))

        # ================= KHU VỰC 3: BẢNG DANH SÁCH & HỆ THỐNG KHÓA VỊ TRÍ =================
        ...
        cols = ("stt", "lock", "name", "len", "wid", "dep", "area", "vol")
        ...
        self.tree.column("lock", width=30, anchor="center")
        self.tree.column("name", width=120, anchor="w")
        self.tree.column("len", width=60, anchor="center")
        self.tree.column("wid", width=60, anchor="center")
        self.tree.column("dep", width=60, anchor="center")
        self.tree.column("area", width=60, anchor="center")
        self.tree.column("vol", width=60, anchor="center")
        ...
        # --- HÀNG NÚT CÔNG CỤ CƠ BẢN (ĐÃ FIX LẸM CHỮ BẰNG AUTO EXPAND) ---
        btn_bar = tk.Frame(right_panel, bg="#f5f6fa")
        btn_bar.pack(fill=tk.X, padx=10, pady=(2, 2))
        
        tk.Button(btn_bar, text="▲ Lên", font=("Segoe UI", 9, "bold"), bg="#dfe6e9", cursor="hand2", padx=8, pady=4, relief=tk.FLAT, command=self.move_up).pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        tk.Button(btn_bar, text="▼ Xuống", font=("Segoe UI", 9, "bold"), bg="#dfe6e9", cursor="hand2", padx=8, pady=4, relief=tk.FLAT, command=self.move_down).pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        tk.Button(btn_bar, text="✏️ Sửa Tên", font=("Segoe UI", 9, "bold"), bg="#dfe6e9", cursor="hand2", padx=8, pady=4, relief=tk.FLAT, command=self.on_edit_popup).pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        tk.Button(btn_bar, text="🗑️ Xóa", font=("Segoe UI", 9, "bold"), bg="#d63031", fg="white", cursor="hand2", padx=8, pady=4, relief=tk.FLAT, command=self.delete_item).pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)

        # --- HÀNG NÚT KHÓA HÌNH VẼ ĐỘC LẬP ---
        btn_bar_lock = tk.Frame(right_panel, bg="#f5f6fa")
        btn_bar_lock.pack(fill=tk.X, padx=10, pady=(2, 8))
        
        tk.Button(btn_bar_lock, text="🔒/🔓 Khóa khung bao", font=("Segoe UI", 9, "bold"), bg="#f39c12", fg="white", cursor="hand2", relief=tk.FLAT, pady=4, command=self.toggle_item_lock).pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        tk.Button(btn_bar_lock, text="🔒 Khóa All", font=("Segoe UI", 9, "bold"), bg="#e17055", fg="white", cursor="hand2", relief=tk.FLAT, pady=4, command=self.lock_all).pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        tk.Button(btn_bar_lock, text="🔓 Mở All", font=("Segoe UI", 9, "bold"), bg="#00b894", fg="white", cursor="hand2", relief=tk.FLAT, pady=4, command=self.unlock_all).pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)

    # ================== HỆ THỐNG KHÓA VỊ TRÍ HÌNH VẼ (MỚI) ==================

    def toggle_item_lock(self):
        if self.selected_item_idx is None:
            messagebox.showinfo("Thông báo", "Vui lòng chọn một Hạng mục để Khóa / Mở khóa!")
            return
        it = self.items[self.selected_item_idx]
        it["is_locked"] = not it.get("is_locked", False)
        
        status = "BỊ KHÓA 🔒" if it["is_locked"] else "ĐÃ MỞ 🔓"
        self.lbl_status.config(text=f"Trạng thái: '{it['name']}' {status}")
    ...
    def lock_all(self):
        ...
        self.reindex_table()
        self.render_canvas(zoom_changed=True)
        self.lbl_status.config(text="Trạng thái: ĐÃ KHÓA TOÀN BỘ HẠNG MỤC 🔒")
    ...
    def unlock_all(self):
        ...
        self.reindex_table()
        self.render_canvas(zoom_changed=True)
        self.lbl_status.config(text="Trạng thái: ĐÃ MỞ KHÓA TOÀN BỘ 🔓")
    ...
    # ================== XỬ LÝ Ổ KHÓA BƯỚC 2 (Khóa Tỉ Lệ Đo) ==================
    ...
    def apply_manual_edit_and_update(self):
        ...
        it = self.items[self.selected_item_idx]
        if it.get("is_locked", False):
            messagebox.showwarning("Cảnh báo", "Hạng mục này đang bị Khóa 🔒\n\nVui lòng BẤM NÚT MỞ KHÓA trước khi muốn chỉnh sửa số liệu trực tiếp!")
            return
    ...
    def create_item_canvas_objects(self, idx, it):
        ...
        is_locked = it.get("is_locked", False)
        
        if is_locked:
            color = "#f39c12" if is_selected else "#e67e22" 
        else:
        ...
        lbl = f"[{it['stt']}] {lock_icon}{it['name']} ({it['length']}x{it['width']}m)"
        ...
        if is_selected and not is_locked:
            ...
            for h_type, (hx, hy) in handles.items():
                hid = self.canvas.create_rectangle(hx - HANDLE_SIZE/2, hy - HANDLE_SIZE/2,
                                                   ...
                                                   fill="#ffffff", outline=color, width=LINE_WIDTH)
    ...
    def create_calib_canvas_objects(self):
        ...
        self.calib_text_id = self.canvas.create_text(min_x + 6, min_y + 6, anchor="nw", text="[MẪU CHUẨN NÚM TO]", fill="#e17055", font=("Segoe UI", 9, "bold"))
    ...
    def sync_item_ui(self, idx):
        ...
        if is_locked:
            color = "#f39c12" if is_selected else "#e67e22" 
            text_color = "#ffeaa7" if is_selected else "#ffffff"
        else:
            color = "#00cec9" if is_selected else "#0984e3" 
            text_color = "#55efc4" if is_selected else "#ffffff"
            
        self.canvas.itemconfig(it['rect_id'], outline=color)
        self.canvas.itemconfig(it['text_id'], fill=text_color)
        
        lock_icon = "🔒 " if is_locked else ""
        lbl = f"[{it['stt']}] {lock_icon}{it['name']} ({it['length']}x{it['width']}m)"
        self.canvas.itemconfig(it['text_id'], text=lbl)
        
        if 'handle_ids' in it and is_selected and not is_locked:
            handles = self.get_handle_coords(min_x, min_y, max_x, max_y)
            for h_type, (hx, hy) in handles.items():
    ...
    def set_mode(self, mode):
        ...
        if mode == "CALIBRATE":
            ...
            self.lbl_status.config(text="Chế độ: ĐIỀU CHỈNH MẪU CHUẨN (Kéo núm cam hoặc di dời khung)")
        else:
            ...
            self.lbl_status.config(text="Chế độ: ĐO ĐỐI TƯỢNG (Kéo chuột vẽ ô mới hoặc bấm chọn ô để chỉnh)")
        self.render_canvas(zoom_changed=True)
    ...
    def get_hit_handle_for_box(self, sx, sy, x1, y1, x2, y2):
        ...
        handles = self.get_handle_coords(min_x, min_y, max_x, max_y)
        for h_type, (hx, hy) in handles.items():
            if abs(sx - hx) <= HANDLE_RADIUS and abs(sy - hy) <= HANDLE_RADIUS:
                return h_type
    ...
    def on_mouse_hover(self, event):
        ...
        if self.mode == "CALIBRATE" and self.calib_coords:
            ...
            if not it.get("is_locked", False):
                ...
                if hit_idx == self.selected_item_idx:
                    h = 'locked'
        ...
        elif h == 'locked': self.canvas.config(cursor="arrow") 
            ...
            if h:
                ...
                if hit_idx is not None:
                    ...
                    if not it.get("is_locked", False):
                        ...
                        self.active_handle = 'locked_click'
                        self.drag_target = None
                else:
                    self.selected_item_idx = None
                    self.active_handle = 'new_draw'
    ...
    def on_mouse_move(self, event):
        ...
        elif self.active_handle and self.orig_box_before_drag and self.active_handle != 'locked_click':
        ...
        if self.active_handle == 'new_draw':
            ...
            if pw > 4 and ph > 4:
                if self.drag_target == 'CALIB':
                    ...
                    name = f"Hạng Mục {stt:02d}"
                    depth = 0.0
                    try: depth = float(self.ent_depth.get().strip())
                    except: pass
                    
                    item = {"stt": stt, "name": name, "is_locked": False, "x1": x1, "y1": y1, "x2": x2, "y2": y2,
    ...
    def update_single_tree_row(self, idx):
        if 0 <= idx < len(self.items):
            it = self.items[idx]
            lock_str = "🔒" if it.get("is_locked", False) else ""
            self.tree.item(str(idx + 1), values=(it["stt"], lock_str, it["name"], f"{it['length']:.2f}", f"{it['width']:.2f}",
    ...
    def reindex_table(self):
        ...
        for i, item in enumerate(self.items, 1):
            item["stt"] = i
            lock_str = "🔒" if item.get("is_locked", False) else ""
            self.tree.insert("", tk.END, iid=str(i),
                             values=(i, lock_str, item["name"], f"{item['length']:.2f}", f"{item['width']:.2f}",
    ...
    def on_edit_popup(self, event=None):
        ...
        if item.get("is_locked", False):
            messagebox.showwarning("Cảnh báo", "Hạng mục này đang bị Khóa 🔒\n\nVui lòng BẤM NÚT MỞ KHÓA trước khi muốn đổi tên!")
            return
            
        top = tk.Toplevel(self.root)
        top.title("Sửa thông tin Hạng mục")
        ...
        tk.Button(top, text="Lưu thay đổi (Enter)", font=("Segoe UI", 9, "bold"), bg="#00b894", fg="white", command=save).pack(fill=tk.X, padx=15, pady=18)
        
        top.bind("<Return>", save)
        e_name.focus_set()
        e_name.select_range(0, tk.END)

    # ================== XUẤT EXCEL & ẢNH SIÊU NÉT (TÁCH LAYER TRÁNH ĐÈ CHỮ) ==================
    ...
    def export_annotated_image(self):
        ...
        try:
            ...
            # --- PHASE 1: VẼ TOÀN BỘ KHUNG ĐỎ TRƯỚC (Layer Dưới Cùng) ---
            for it in self.items:
                x1, y1, x2, y2 = it["x1"], it["y1"], it["x2"], it["y2"]
                min_x, max_x = min(x1, x2), max(x1, x2)
                min_y, max_y = min(y1, y2), max(y1, y2)
            ...
            # --- PHASE 2: VẼ TOÀN BỘ TAG CHỮ VÀNG SAU (Layer Trên Cùng) ---
            for it in self.items:
                x1, y1, x2, y2 = it["x1"], it["y1"], it["x2"], it["y2"]
                min_x, max_x = min(x1, x2), max(x1, x2)
                min_y, max_y = min(y1, y2), max(y1, y2)
                
                lbl = f" [{it['stt']}] {'🔒 ' if it.get('is_locked') else ''}{it['name']} ({it['length']}x{it['width']}m) "
                try:
                ...
                # Thuật toán né đè chữ
                collision = True
                attempts = 0
                while collision and attempts < 100:
                    collision = False
                    for bx1, by1, bx2, by2 in placed_tags:
                        if not (tag_x2 < bx1 or tag_x1 > bx2 or tag_y2 < by1 or tag_y1 > by2):
                            tag_y2 = by1 - 2
                ...
                placed_tags.append((tag_x1, tag_y1, tag_x2, tag_y2))
